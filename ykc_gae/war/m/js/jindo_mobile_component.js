/** Jindo Mobile Component(minify:0) : ../www/docs/jindo-mobile/archive/1.5.0/
 *	jindo.m
 *	jindo.m.Component
 *	jindo.m.Transition
 *	jindo.m.UIComponent
...
jindo.m = (function() {
    ...
    function _initDeviceInfo() {
        ...
        _htDeviceInfo = {
            ...
            "bInapp" : false,
            "version" : "",
            "samsung" : f('GT-', sName) || f('SCH-', sName) || f('SHV-', sName)||f('SHW-', sName) ||f('SPH', sName) || f('SWT-', sName) ||f('SGH-', sName) || f("EK-", sName),
            "lg" : f('LG-', sName),
            "pantech" : f('IM-', sName)
        };
    ...
    var __M__ = {
        ...
        **/
        _isUseTimingFunction : function() {
            ...
            if(_htDeviceInfo.android || ((_htDeviceInfo.iphone || _htDeviceInfo.ipad) && (parseInt(_htDeviceInfo.version,10) == 6) )) {
                bUse = false;
            }
            return bUse;
        },
...
jindo.m.Component.getInstance = function(){
	...
	- css timing function을 구현한 jindo.m.TimingFunction을 추가함
	- clear() 함수 추가
	<1.1.1 릴리즈>
	- bUseCss3d 옵션추가
	- bUseCss3d -> bUseTimingFunction
	<1.5.0 릴리즈>
	- requestAnimationFrame/cancelAnimationFrame 추가
**/
/**
...
/**
    ...
    @history 1.4.0 Supporti OS 6 지원
...
플리킹 이동을 이 위치만큼 이동하게 수정가능한 옵션
            ...
            @param {Function} [htOption.bUseCss3d=jindo.m._isUseCss3d(true)] css3d(translate3d) 사용여부<br />
                모바일 단말기별로 다르게 설정된다. 상세내역은 <auidoc:see content="jindo.m">[jindo.m]</auidoc:see>을 참조하기 바란다.
            @param {Function} [htOption.bUseTimingFunction=jindo.m._isUseTimingFunction()] 애니메이션 동작방식을 css의 TimingFunction을 사용할지 여부<br />false일 경우 setTimeout을 이용하여 애니메이션 실행.<br />
            모바일 단말기별로 다르게 설정된다. 상세내역은 <auidoc:see content="jindo.m">[jindo.m]</auidoc:see>을 참조하기 바란다.
    ...
    **/
    $init : function(sId, htUserOption){
     ...
     $static : {
        _htAnimation : {
            'circular-slide' : 'jindo.m.CircularSlideFlicking',
            'slide' : 'jindo.m.SlideFlicking',
            'cover' : 'jindo.m.CoverFlicking',
            'circular-cover' : 'jindo.m.CoverFlicking',
            'flip' : 'jindo.m.FlipFlicking',
            'circular-flip' : 'jindo.m.FlipFlicking',
            'alignFlip' : 'jindo.m.AlignFlipFlicking',
            'circular-alignFlip' : 'jindo.m.AlignFlipFlicking'
        }
    },
    ...
    **/
    _initVar: function() {
        ...
        this._htIndexInfo = {
            nContentIndex : 0,
            nNextContentIndex : 0,
            welElement :  this._htWElement.aPanel[0],
            welNextElement : this._htWElement.aPanel[0],
            sDirection : null
        };
        ...
        //this._bDummyTagException = (this._bAndroid && (this._nVersion < "3") ) || (this._bAndroid && (this._nVersion === "4.0.4") ) || this._b3dExecption;
        this._bDummyTagException = (this._bAndroid && (this._nVersion < "3") ); //|| this._b3dExecption;

    },

    /**
        jindo.m.Flicking 에서 사용하는 모든 엘리먼트의 참조를 가져온다.
    **/
    ...
    **/
    ...
    /**
        touchStart 커스텀 이벤트 처리
    **/
    _onStart : function(oCustomEvt){
        //alert('start');
        //console.log('[STARt] ' + this._doFlicking);
        if (this._doFlicking) {
            return;
        }
       ...
       if(!this.fireEvent('touchStart', oCustomEvt)){
        ...
        if(this._oFlickingAnimation){
            this._oFlickingAnimation.onStart();
        }
    },

    /**
        touchMove 커스텀 이벤트 처리
     */
    _onMove : function(oCustomEvt){
        var bH = this.option('bHorizontal');
        /** 시스템 스크롤 막기 */
        var weParent = oCustomEvt.oEvent;
        if(oCustomEvt.sMoveType === jindo.m.MOVETYPE[0]) {  //수평이고,수평스크롤인 경우 시스템 스크롤 막기
            if(bH) {
        ...
        } else if(oCustomEvt.sMoveType === jindo.m.MOVETYPE[1]) {   //수직이고, 수직스크롤인 경우 시스템 스크롤 막기
            if(!bH) {
                weParent.stop(jindo.$Event.CANCEL_ALL);
            }else{
                this.fireEvent('scroll');
        ...
        **/
        ...
        if(this._oFlickingAnimation){
            //방향, 거리, 패널정보
             this._oFlickingAnimation.movePanel(nDis, nVector, nPos, 0 );
        }

    },

    /**
        touchEnd 커스텀 이벤트 핸들러
     */
     _onEnd : function(oCustomEvt){
        ...
        if(htInfo.sDirection === null){
            ...
            if(nDis === 0 || ((oCustomEvt.sMoveType === jindo.m.MOVETYPE[2]) && !this.option('bUseDiagonalTouch')) ) {
                //console.log('call endAnimation');
        ...
        if(htInfo.sDirection !== null){
            ...
            **/
            if(!this.fireEvent('beforeFlicking', htParam)){
                this._oFlickingAnimation.restorePosition();
                return;
            }
        }
        ...
        if(this._oFlickingAnimation){
            this._oFlickingAnimation.movePanel( nDis, nVector,nPos, nTime, true);
        }
        /**
    ...
    **/
    _getSnap : function(nDistanceX, nDistanceY, nDuration, sType){
        ...
        //가로대가석일 경우
        if(! ((sType === jindo.m.MOVETYPE[2]) && !this.option('bUseDiagonalTouch') )){
    ...
    **/
    _createAnimation : function(sType){
        if(jindo.m.Flicking._htAnimation[sType]) {
            ...
            //console.log("객체 생성 : new " +jindo.m.Flicking._htAnimation[sType] + "()" );
            try{
    ...
    **/
    setFlickingAnimation : function(sType){
        ...
        if(!this._oFlickingAnimation || (this.option('sAnimation') !== sType)){
            ...
            if(this._oFlickingAnimation){
                this._oFlickingAnimation.setOption(this.option());
    ...
    **/
    ...
    /**
        ...
        @param {Boolean} flicking 커스텀이벤트 발생여부
    **/
    _moveTo : function(nIndex, nDuration , bFireEvent, bFireFlickEvent){
        if(typeof nDuration === 'undefined'){
            nDuration = this.option('nDuration');
        }
        ...
        if(typeof bFireFlickEvent === 'undefined'){
            bFireFlickEvent  = false;
        }
    ...
    **/
    ...
    _onScroll : function(evt){
        if(this._doFlicking){
              ...
              setTimeout(function(){
                  if(self._doFlicking){
                        var n = self._htIndexInfo.nNextContentIndex;
     ...
     _onActivate : function() {
         ...
         this._setAnchorElement();
         if(this._oFlickingAnimation){
             if(typeof this._oFlickingAnimation._oCore !== 'undefined'){
                 this._oFlickingAnimation._oCore.activate();
                 //this._oFlickingAnimation = null;
             }else{
                this._oTouch.activate();
             }
    ...
    /**
     ...
     _onDeactivate : function() {
         ...
         if(this._oFlickingAnimation){
             if(typeof this._oFlickingAnimation._oCore !== 'undefined'){
                 this._oFlickingAnimation._oCore.deactivate();
             }
         }
         this._oTouch.deactivate();
    ...
    **/
    _attachEvent : function() {
       ...
       /* scroll이벤트 */
       if(this._isIos && parseInt(this._nVersion,10) === 6 ){
           this._htEvent["scroll"] = {
               fn : jindo.$Fn(this._onScroll, this).bind(),
               el : window
           };
...
jindo.m.FlickingAnimation = jindo.$Class({
    ...
    */
    setOption : function(htOption){
      ...
      for(var sKey in htOption) {
                    this._htOption[sKey] = htOption[sKey];
              }
              
        this.bH = this._htOption['bHorizontal'];
    },
      
    setWrapperElement : function(htWElement){
         this._htWElement = {};
         for(var p in htWElement){
             this._htWElement[p] = htWElement[p];
         }
    },
    ...
    moveTo : function(n, nTime, bFireEvent, bFireFlickEvent){
        this._moveTo(n, nTime, bFireEvent, bFireFlickEvent);
    },
    
    _moveTo : function(n, nTime,  bResize,  bFireMoveEvent, bFireFlickEvent){
        
    },
    
    refresh : function(n, bResize){
        this._refresh(n, bResize);
    },
    
    movePanel : function(nDistacne, nVector, nPos, nDuration, bEnd){
        this._movePanel(nDistacne, nVector, nPos, nDuration, bEnd);
    },
    
 
    setInstance : function(oIns){
        this._WrapperInstacne = oIns;
    },
    
    endAnimation : function(bFireEvent){
        if(typeof bFireEvent === 'undefined' ){
            bFireEvent = true;
        }
   ...
   /**
    ...
    _attachTransitionEnd : function(el, nTime){
          if(el === this._WrapperInstacne.getElement().$value() || el === this._htWElement.container.$value()){
    ...
    @fileOverview  AlignFlip flicking 플러그인 
    @author "oyang2"
    @version 1.5.0
    @since  2012.08.01
...
/**
   AlignFlip flicking 플러그인 
    ...
    @keyword flip, alignFlipFlicking
    @group Component
**/

jindo.m.AlignFlipFlicking = jindo.$Class({ 
    /** @lends jindo.m.AlignFlipFlicking.prototype */
   /**
        초기화 함수
    **/
    ...
    _initAnimation : function(){
         ...
         this._sAlign = this._htOption['sFlipAlign'];
         this._prepareFlip();
        
    },
     /**
     * @description 플립하기 전에 필요한 Perspective 세팅한다 
     */
    _prepareFlip : function(){
     ...
     /**
     * @description 플리킹에 필요한 스타일을 추가한다.
     */
     ...
     * @description 화면 전환
     *  @param {Number}  n 
     *  @param {Boolean} 화면 리사이즈 여부
     *  @param {Boolean} 커스텁 이벤트 발생 여
     */
    _refresh : function(n, bResize, bFireEvent){
        if(typeof n === 'undeinfed'){
            n = this._WrapperInstacne.getContentIndex();
        }
        var nCenter = n;
        if(this._htOption['bUseCircular']){
            nCenter = n%3;
        }
        ...
        this._WrapperInstacne._htIndexInfo.nContentIndex = n;
    ...
    _setFlipElement : function(){
        ...
        var welNext = (this._bMoveDirection === "prev")? this._WrapperInstacne.getPrevElement():this._WrapperInstacne.getNextElement();
    ...
    _getFlipElement : function(){
        var welCurrent = this._WrapperInstacne.getContentElement();
        if(this._bMoveDirection === "prev"){
             if(this._htOption['sFlipAlign'] === "left" || this._htOption['sFlipAlign']=="top"){
                 welCurrent = this._WrapperInstacne.getPrevElement();
             }
        }else if(this._bMoveDirection === "next"){
             if(this._htOption['sFlipAlign'] === "right" || this._htOption['sFlipAlign']=="bottom"){
                 welCurrent = this._WrapperInstacne.getNextElement();
             }
        }
    ...
    /**
     * @description 플립되고 뒤에 화면에 보여야 하는 엘리먼트를 리턴한다
     * @return {Element}
     */
     _getFlipNextElement : function(){
        var welNextElement = this._WrapperInstacne.getNextElement();
        
       if(this._bMoveDirection === 'next'){
              if(this._htOption['sFlipAlign'] == "right" || this._htOption['sFlipAlign'] ==="bottom"){
                    welNextElement = this._WrapperInstacne.getContentElement();
              }
         ...
         return welNextElement;
    },
    
    
    /**
     * @description touchmove, touchend 시점에 호출
     * @param {Number} nDistance
     * @param {Number}  nVector
     * @param {Number} nPos
     * @param {Number} nDuration
     * @param {Boolean} bEnd
     */ 
    _movePanel : function(nDistance, nVector, nPos, nDuration , bEnd){
         if(bEnd){
             if(this._bMove){
                 this._onEnd();
             }
              ...
              if(this._bMoveDirection === null){
                  ...
                  this._WrapperInstacne._restoreAnchor();      
                  this._WrapperInstacne._setAnchorElement();
                  this._WrapperInstacne._clearAnchor();
                  
              }else{
                  //움직여라 플릭을 진행하여라;
                  var nSize = this.bH? this.nWidth : this.nHeight;
                  var welTarget = this._WrapperInstacne.getContentElement();
            
                  if(this._bMoveDirection === 'prev'){
                      if(this._htOption['sFlipAlign'] ==="left" || this._htOption['sFlipAlign'] === "top"){
                            welTarget  = this._WrapperInstacne.getPrevElement();
                      }
                      
                      this._nDeg =Math.max(0, Math.min(70, Math.round((nDistance * 1/(nSize)) * 70)));
                     
                      this._setPrevRotate(welTarget, this._nDeg);
                      
                  }else if(this._bMoveDirection === 'next'){
                      if(this._htOption['sFlipAlign'] ==="right" || this._htOption['sFlipAlign'] === "bottom"){
                            welTarget  = this._WrapperInstacne.getNextElement();
                      }
    ...
    _setPrevRotate : function(welTarget, nDeg){
        ...
        if(this._htOption['sFlipAlign'] === "right"){
            nCurrentDeg = Math.min(90, nCurrentDeg);
        }else if(this._htOption['sFlipAlign'] === "bottom"){
            nCurrentDeg = Math.max(-90, nCurrentDeg);
        }else if(this._htOption['sFlipAlign'] === "left"){
        ...
        //nCurrentDeg = Math.min(0, nCurrentDeg);
        //console.log('prev : '+ nDeg , nCurrentDeg, welTarget.$value());
    ...
    _setNextRotate : function(welTarget, nDeg){
        ...
        if(this._htOption['sFlipAlign'] === "top"){
            nCurrentDeg = Math.min(90, nCurrentDeg);
        }else if(this._htOption['sFlipAlign'] === "left"){
            nCurrentDeg = Math.max(-90, nCurrentDeg);
        }else if(this._htOption['sFlipAlign'] === "bottom"){
            CurrentDeg = Math.min(0, nCurrentDeg);
        } else{
            nCurrentDeg = Math.max(0, nCurrentDeg);
        }
        //nCurrentDeg = Math.min(0, nCurrentDeg);
        
        //console.log('next : '+ nDeg , nCurrentDeg);
    ...
    _setFlipAlign : function(wel){
        var sAlign = "";
        if(this.bH){
            sAlign = this._htOption['sFlipAlign'] + " center";
        }else{
            sAlign =  "center " + this._htOption['sFlipAlign'];
        }
    ...
    _rotate : function(wel, nDeg){
        if(wel){
            var sRotate = this.bH?  "rotateY(" + nDeg + "deg)" : "rotateX(" + nDeg + "deg)";
            var htCss = {};
            ...
            // wel.css({
                ...
                // "WebkitTransform" : sRotate + " rotateZ(0deg)"
            // });
        }
    ...
    /**
     *  @description touchEnd에 대한 구
     */
    _onEnd : function(){
        this._bFinished = false;
        var bRepos = (this._WrapperInstacne._htIndexInfo.sDirection === null)? true : false;
        if(this._bMoveDirection !== this._WrapperInstacne._htIndexInfo.sDirection){
            bRepos = true;
            this._WrapperInstacne._htIndexInfo.sDirection = null;
            this._WrapperInstacne._htIndexInfo.nNextContentIndex = this._WrapperInstacne._htIndexInfo.nContentIndex;
            this._WrapperInstacne._htIndexInfo.welNextElement = this._WrapperInstacne._htIndexInfo.welElement;
        }
        
        if(this._bMoveDirection === "prev" && (this._WrapperInstacne.getElement().$value() === this._WrapperInstacne.getPrevElement())){
            bRepos = true;
            this._WrapperInstacne._htIndexInfo.sDirection = null;
            this._WrapperInstacne._htIndexInfo.nNextContentIndex = this._WrapperInstacne._htIndexInfo.nContentIndex;
            this._WrapperInstacne._htIndexInfo.welNextElement = this._WrapperInstacne._htIndexInfo.welElement;
        ...
        if(this._bMoveDirection === "next" && (this._WrapperInstacne.getElement().$value() === this._WrapperInstacne.getNextElement().$value())){
            bRepos = true;
            this._WrapperInstacne._htIndexInfo.sDirection = null;
            this._WrapperInstacne._htIndexInfo.nNextContentIndex = this._WrapperInstacne._htIndexInfo.nContentIndex;
            this._WrapperInstacne._htIndexInfo.welNextElement = this._WrapperInstacne._htIndexInfo.welElement;
       ...
       // console.log()
        ...
        var nDuration = bRepos? this._htOption['nBounceDuration'] : this._htOption['nDuration'];
    ...
    _resetInfo : function(){
     ...
     /**
     * @description _onTransitionEnd
     */
    _onTransitionEnd : function(){
        this._detachTransitionEnd();
        var bFireEvent = true;
        if(this._WrapperInstacne._htIndexInfo.sDirection === null){
            bFireEvent = false;
        }
 ...
 * @param {Number} nDuration 애니메이션 시간 
   ...
   movePrev : function(nDuration){
       ...
       if(welPrev.$value() === this._WrapperInstacne.getElement().$value()){
           return;
       }
       this._bMoveDirection = 'prev';
       this._setFlipElement();
       ...
       this._nDeg = 1;
        ...
        var n = this._htOption['nFlickThreshold'];
        this._bMoveDirection  = 'prev';
        this._WrapperInstacne._onEnd({
            nDistanceX : n+10,
            nDistanceY : n+10,
            nX : 10,
            nY : 10
 ...
 * @param {Number} nDuration 애니메이션 시간 
    ...
    moveNext : function(nDuration){
       ...
       if(welNext.$value() === this._WrapperInstacne.getElement().$value()){
           return;
       }
       this._bMoveDirection = 'next';
       this._setFlipElement();
       ...
       var n = this._htOption['nFlickThreshold']*-1;
       var nPos = this.bH? this._htWElement.base.width() :  this._htWElement.base.height();
...
/**
    @fileOverview  Circular Slide flicking 플러그인 
    @author "oyang2"
    @version 1.5.0
    @since  2012. 05. 24
...
**/
/**
   Circular Slide flicking 플러그인 

    @class jindo.m.CircularSlideFlicking
    @invisible
    @extends  jindo.m.FlickingAnimation
    @keyword circulr, slide, circular-slide 
    @group Component
**/
jindo.m.CircularSlideFlicking = jindo.$Class({
    /** @lends jindo.m.CircularSlideFlicking.prototype */
     /**
        초기화 함수
    **/
    ...
    $init : function(){
           this._isIos = jindo.m.getDeviceInfo().iphone || jindo.m.getDeviceInfo().ipad;
           ...
           this._nLastDis = null;
           //transitionend이후에 translate3d 설정을 제거했다가 다시 추가해야 정상동작하는 기기.
           this._b3dExecption = false;
           
    },
    
    /**
     * @description 플리킹실행전 세팅해야 하는 부분
     */
    _initAnimation : function(){
        this._bUseCss3d = this._htOption['bUseCss3d'];
   ...
   /**
    * @description 각 패널과 컨테이너의 위치를 잡는다.
    */ 
    ...
    _prepareTransition : function(){
        ...
        if(this._htOption['bUseCss3d']){
             this._sTranslateStart = "translate3d(";
             this._sTranslateEnd = ",0px)";
        }
        
        //var sTransfrom = this._htOption['bUseTranslate']? "-webkit-transform" : (this.bH? 'left' : 'top');
        var sTransfrom = "all";
    ...
    _refresh : function(n, bResize, bFireEvent){
          ...
          if(this._b3dExecption){
                if((this._htOption['bUseTranslate'] || this._htOption['bUseCss3d'])){
                     var sValue = "";
          ...
          if (this._htOption['bUseTranslate']) {
                if(this._b3dExecption){
                     setTimeout(function(){
                         self._htWElement.container.css(self._sCssPrefix + 'Transform', self._sTranslateStart + "0,0px" + self._sTranslateEnd);
                     },50);
                 }else{
                    this._htWElement.container.css(this._sCssPrefix + 'Transform', this._sTranslateStart + "0,0px" + this._sTranslateEnd);
                }
           }
    ...
    /**
     * @description 각 패널을 움직인다.
 * @param {Number} nDistance touchstart시점부터 현재까지 움진인 거
 * @param {Number} nVector 직전 touch 시점과의 상대 거리
 * @param {Number} nPos 현재 touch 지점의 좌표 
 * @param {Number} nDuration 애니메이션 시간
 * @param {Boolean} bEnd touchEnd시점 여부  
      */
    _movePanel : function(nDistance, nVector, nPos, nDuration , bEnd){
        var self = this;
        //console.log('*---- ', nDistance, nVector, nPos, nDuration , bEnd);
        ...
        //옵션설정이 되었을 경우 
        if(this._htOption['bSetNextPanelPos'] && (Math.abs(nDistance) >5 )){
        ...
        //if(!bEnd || (this._htOption['bUseTimingFunction'])  || (nDuration === 0) || (this._WrapperInstacne._htIndexInfo.sDirection === null)){
        if(!bEnd || (this._htOption['bUseTimingFunction'])  || (nDuration === 0) ){
            this._setPosition(welWrapper, nDistance, nVector, nPos, nDuration, bEnd);
        }else{
            //스크립트 방식으로 애니메이션 처리;
            nDistance =  this._nLastDis?this._nLastDis :  nDistance;
             var startTime = (new Date()).getTime(),
                 nStartDis =  nDistance, nBeforeDis = nDistance, nStartVector = nVector, nTotalDis = this.bH? this._htWElement.base.width(): this._htWElement.base.height();
                ...
                if(!this._htOption['bUseTranslate']){
                 ...
                 if(nDistance < 0){
                    ...
                    (function animate () {
                       ...
                       //console.log('CALL AnimationFrame : ' + nDis + " // " + (nDis-nBeforeDis));
    ...
    /**
     * @description 제자리로 위치를 이동하는 함수 
     */
    restorePosition : function(){
        var nTime = this._htOption['nBounceDuration'];
 ...
 * @param {Number} nDruation  애니메이션 시간
 * @param {Boolean} bFireEvent move 커스텀 이벤트 발생여부
 * @param {Boolean} bFireFlickEvent afterFlicking 커스텀 이벤트 발생여부
     */
    _moveTo : function(n, nDruation, bFireEvent, bFireFlickEvent){
        this._WrapperInstacne.refresh(n, false, bFireEvent);
    },
    
    /**
     * @description 애니메이션이 완료된 이후에 실행.
     */
   _onTransitionEnd : function(){
        this._detachTransitionEnd(); 
 ...
 * @param {Number} nDuration 애니메이션 시간 
 * @param {Boolean} bEnd  touchEnd 시점여부 
     */
    _setPosition : function(wel, nDis, nVector, nPos, nDuration, bEnd){
        if(typeof nDuration === 'undefined'){
            nDuration = 0;
        }        
       if(!this._htOption['bUseTranslate']){
           this._setPositionForStyle(wel, nDis, nVector , nDuration, bEnd);
       }else{
            this._setPositionTransform(wel, nDis, nDuration, bEnd);
       }
 ...
 * @param {Boolean} bEnd 현재 touchEnd 시점여부 
     */
    _setPositionForStyle : function(wel, nDis, nVector, nDuration, bEnd){
        var sName = this.bH? 'left' : 'top';
        ...
        htCss[sName]  = nPos+ '%';
        //htCss[this._sCssPrefix+'Transform'] = 'translate(' + nX +"px,"+nY+"px" +')';
        
        this._nLastDis = nDis;
       if(bEnd){
            if(nPos === parseFloat(wel.css(sName).replace('px',''),10) ){
                nDuration = 0;
            }
 ...
 * @param {Boolean} bEnd 현재 touchEnd 시점여부 
     */
    _setPositionTransform : function(wel, nDis, nDuration, bEnd){
        if(bEnd){
            if(this._WrapperInstacne._htIndexInfo.sDirection === null){
                nDis = 0;
            }else{
                if(nDis < 0){
                    nDis =  this._htContainerInfo.left;
                }else{
                    nDis =  this._htContainerInfo.right;
                }
            }
        }
       ...
       this._nLastDis = nDis;
        ...
        if(bEnd){
            var htCssOffset = jindo.m.getCssOffset(wel.$value());
            if((htCssOffset.left === nX) && (htCssOffset.top === nY)){
                nDuration = 0;
            }
            //console.log('____ ATTACH2 ' +  nDuration + " : " + wel.css(this._sCssPrefix+'Transform')  + " : " + (this._sTranslateStart + nX +"px,"+nY+"px" + this._sTranslateEnd));
            this._attachTransitionEnd(wel.$value(), nDuration);
        }
    ...
    @fileOverview  Cover flicking 플러그인 
    @author "oyang2"
    @version 1.5.0
    @since  2012. 05. 24
...
jindo.m.CoverFlicking = jindo.$Class({
    ...
    **/
    ...
    /**
     * @description 커버플리킹만의 기본 옵션을 설정하기 위한 코드
     */
    _initOption : function(){
        for(var p in this._htDefaultOption){
            if(typeof this._htOption[p] === 'undefined'){
                this._htOption[p]  = this._htDefaultOption[p];
            }
        }
        
        this._bUnableTouchMove = !this._htOption['bUseMoveEffect'];
        if(this._htOption['bUseCss3d']){
             this._sTranslateStart = "translate3d(";
             this._sTranslateEnd = ",0px)";
             
             this._sScaleStart = "scale3d(";
             this._sScaleEnd = ",1)";
        }
    },
     /**
 ...
 * @param {Boolean} bEnd touchEnd시점 여부  
    ...
    _movePanel : function(nDistance, nVector, nPos, nDuration , bEnd){
        ...
        if(this._bMoveDirection  === null && (Math.abs(nDistance) > 5) ){
               if(nDistance > 0){
                   this._bMoveDirection = 'prev';
               }else{
                   this._bMoveDirection = 'next';
               ...
               var welZoom = (this._bMoveDirection  === 'prev')? this._WrapperInstacne.getPrevElement() : this._WrapperInstacne.getNextElement();
               if(welZoom.$value() === this._WrapperInstacne.getElement().$value()){
                   this._bMoveDirection = null;
                   welZoom = null;
               }else{
        ...
        if( !this._htOption['bUseTimingFunction'] && (nDuration !== 0) && (bEnd)){
            //스크립트 방식으로 애니메이션 처리
             var startTime = (new Date()).getTime(),
                 nStartDis = nDistance, nBeforeDis = nDistance, nBeforePos = nPos,  nTotalDis = this.bH? this._htWElement.base.width(): this._htWElement.base.height();
                 if(this._bMoveDirection === null || ((this._bMoveDirection === 'prev') && (nDistance < 0) ) || ((this._bMoveDirection === 'next') && (nDistance > 0) )){
                     nTotalDis = 0;
                     this._WrapperInstacne._htIndexInfo.nNextContentIndex = this._WrapperInstacne._htIndexInfo.nContentIndex;
                     this._WrapperInstacne._htIndexInfo.welNextElement = this._WrapperInstacne._htIndexInfo.welElement;
                     this._WrapperInstacne._htIndexInfo.sDirection = null;
                 ...
                 if(nDistance < 0){
                    ...
                    (function animate () {
                        ...
                        self._setPosition(welCenter, nDis,  (nDis-nBeforeDis), (nBeforePos+ nBeforeDis), 0, false);
                        nBeforeDis = nDis;
                        //self._nTimerAnimate = setTimeout(animate, 1);  
                        self._nTimerAnimate = requestAnimationFrame(animate);    
            })();
        }else{
             if(!this._bUnableTouchMove || bEnd){
              //  console.log('TimingFunction');
                this._setPosition(welCenter, nDistance, nVector, nPos, nDuration, bEnd); 
             }
        }
    },
 ...
 * @param {HTMLElement} welZoom 줌인대상 엘리먼트
    ...
    _prepareZoomAnim : function(welZoom){
        //debugger;
          welZoom.show();
          welZoom.$value().style[this._sCssPrefix+'Transform']  =  this._sScaleStart + this._htOption['nDefaultScale'] + ", "+ this._htOption['nDefaultScale'] + this._sScaleEnd;
          //console.log( this._sScaleStart + this._htOption['nDefaultScale'] + ", "+ this._htOption['nDefaultScale'] + this._sScaleEnd);
    },
 ...
 * @param {Boolean} bEnd  touchEnd 시점여부 
   ...
   _setPosition : function(wel, nDis, nVector, nPos, nDuration, bEnd){
        ...
        if(this._htOption['bUseTranslate']){
            this._setPositionTransform(wel, nDis, nPos, nDuration, bEnd);
          
        }else{
             this._setPositionForStyle(wel, nDis, nVector ,nPos, nDuration, bEnd);
        }
    },
 ...
 * @param {Boolean} bEnd 현재 touchEnd 시점여부 
     */
     _setPositionForStyle : function(wel, nDis,  nVector, nPos, nDuration, bEnd){
         var bReturn = false;
         if(bEnd){
             if(this._WrapperInstacne._htIndexInfo.sDirection !== null){
        ...
        //console.log(welZoom.$value(), htZoomCss);
 ...
 * @param {Boolean} bEnd 현재 touchEnd 시점여부 
     */
    _setPositionTransform : function(wel, nDis, nPos, nDuration, bEnd){
        var bReturn = false;
        if(bEnd){
            if(this._WrapperInstacne._htIndexInfo.sDirection !== null){
                if(nDis < 0 && (this._bMoveDirection === 'next') ){
    ...
    _getZoomElement : function(){
        ...
        if(this._bMoveDirection  === 'prev'){
            welZoom = this._WrapperInstacne.getPrevElement();
        }else if(this._bMoveDirection  === 'next'){
            welZoom = this._WrapperInstacne.getNextElement();
        }
        
        if(welZoom &&  (welZoom.$value() === this._WrapperInstacne.getElement().$value()) ){
            welZoom = null;
        }
 ...
 * @param {Number} nPos 현재 터치 지점
    ...
    _getCalculate : function(nPos){
        var nRange = this.bH? this._htWElement.base.width() :  this._htWElement.base.height();
    ...
    _onTransitionEnd : function(){
        ...
        this._WrapperInstacne._htIndexInfo.nContentIndex = this._WrapperInstacne._htIndexInfo.nNextContentIndex;
       
        var sCss = this.bH? 'left' : 'top'; 
        jindo.$A(this._htWElement.aPanel).forEach(function(value, i, array){
            value.$value().style[this._sCssPrefix +'TransitionDuration'] = null;
            if(this._htOption['bUseTranslate']){
        ...
        //this._refresh(this._WrapperInstacne._htIndexInfo.nContentIndex);
        this._restorePanel(this._WrapperInstacne._htIndexInfo.welNextElement.$value()); 
        this.endAnimation(bFireEvent);
        this._resetInfo();
    },
    ...
    /**
     * @description el이 중앙에 오도록 재배치 한다
 * @param {HTMLElement} el 중앙에 오는 엘리먼트
     */
     _restorePanel : function(el){
         for(var i=0,nLen = this._htWElement.aPanel.length;i<nLen;i++){
        ...
        this._WrapperInstacne._htIndexInfo.welElement = this._WrapperInstacne._htIndexInfo.welNextElement;
 ...
 * @param {Boolean} bFireFlickEvent afterFlicking 커스텀 이벤트 발생여부
     */
    _moveTo : function(n, nDuration, bFireEvent, bFireFlickEvent){
        
        this._WrapperInstacne._htIndexInfo.nNextPanelIndex = n;
        this._WrapperInstacne._htIndexInfo.nNextContentIndex = n;
        var nDis = this._htOption['nFlickThreshold']+10; 
        
        if(n === this._WrapperInstacne.getNextIndex()){
            this._movePanel(nDis*-1, 0, 0, nDuration, true);
            
        }else if(n === this._WrapperInstacne.getPrevIndex()){
             this._movePanel(nDis, 0, 0, nDuration, true);
            
        }else{
            this._refresh(n, false, bFireEvent);
        }
    },
 ...
 * @param {Number} nDuration 애니메이션 시간 
     */ 
   movePrev : function(nDuration){
       var welZoom = this._WrapperInstacne.getPrevElement();
       if(welZoom.$value() === this._WrapperInstacne.getElement().$value()){
           return;
       }
 ...
 * @param {Number} nDuration 애니메이션 시간 
     */
    moveNext : function(nDuration){
       var welZoom = this._WrapperInstacne.getNextElement();
       if(welZoom.$value() === this._WrapperInstacne.getElement().$value()){
           return;
       }
...
/**
    @fileOverview  Flip flicking 플러그인 
    @author "oyang2"
    @version 1.5.0
    @since  2012. 08. 01
    ...
    **/
    ...
    _prepareFlip : function(){
       if(typeof this._htWElement.left === 'undefined'){
           ...
           //this._htWElement.base.css(this._sCssPrefix+'TransformStyle',"preserve-3d");
           var sPrevClass = this._htOption['sClassPrefix'] + "left";
           var sNextClass = this._htOption['sClassPrefix'] + "right";
       ...
       //this.bH? this._htWElement.left.width(this.nCenter) : this._htWElement.left.height(this.nCenter); 
       //this.bH? this._htWElement.right.width(this.nCenter): this._htWElement.right.height(this.nCenter); 
    },
    ...
    _setElementSize : function(){ 
        this.nWidth = this._htWElement.base.width();
        this.nHeight = this._htWElement.base.height();
        this.nCenter = Math.round((this.bH? this.nWidth: this.nHeight) / 2);
    },
    ...
    _refresh : function(n, bResize, bFireEvent){
        ...
        //this.bH? this._htWElement.left.height(this.nHeight) : this._htWElement.left.width(this.nWidth);
        //this.bH? this._htWElement.right.height(this.nHeight): this._htWElement.right.width(this.nWidth); 
        
         this._htWElement.right.css(this.bH?"left" : "top" ,  (this.nCenter) + "px");
         var htCss ={};
    ...
    _setCloneElement : function(){
        ...
        var welShow;
        if( this._bMoveDirection === 'prev'){
             welShow = this._WrapperInstacne.getPrevElement();
             ...
             this._htWElement.right.empty().append(jindo.$Element(welCurrent.$value().cloneNode(true)).css(this.bH?"margin-left" :"margin-top", "-" + (this.nCenter) + "px").width(this.nWidth).height(this.nHeight));
             welShow.before(this._htWElement.left).after(this._htWElement.right);
        }else if( this._bMoveDirection === 'next'){
             welShow = this._WrapperInstacne.getNextElement();
             this._htWElement.left.empty().append(jindo.$Element(welCurrent.$value().cloneNode(true)).show().width(this.nWidth));
             if(welShow.$value() === welCurrent.$value()){
                 this._htWElement.right.empty().append(jindo.$Element(jindo.$('<div></div>')).show().css(this.bH?"margin-left" :"margin-top", "-" + (this.nCenter) + "px").width(this.nWidth).height(this.nHeight));
             }else{
                this._htWElement.right.empty().append(jindo.$Element(welShow.$value().cloneNode(true)).show().css(this.bH?"margin-left" :"margin-top", "-" + (this.nCenter) + "px").width(this.nWidth).height(this.nHeight));
             }
         ...
         if(bEnd){
             if(this._bMove){
              ...
              if(this._bMoveDirection === null){
                  ...
                  //움직여라 플릭을 진행하여라;
                  var nSize = this.bH? this.nWidth : this.nHeight;
                  var welCurrent = this._WrapperInstacne.getContentElement(),welTarget,nAngle;
                  if(this._bMoveDirection === 'prev'){
                      welTarget  = this._WrapperInstacne.getPrevElement();
                      nAngle = (welTarget.$value() === welCurrent.$value())? 60 : 180;
                      this._nDeg =Math.max(0, Math.min(nAngle, Math.round((nDistance * 1/(nSize)) * nAngle)));
    ...
    _setNextRotate : function(welCurrent, welShow, welLeft, welRight, nDeg){
        ...
        var nCurrentDeg = this.bH? -nDeg : nDeg;
        var nShowDeg = this.bH? (180 - nDeg):  -(180 - nDeg);
...
/**
    ...
    @class jindo.m.DynamicScrollPlugin
    @keyword scroll, 스크롤
    @group Component
    @invisible
**/
jindo.m.DynamicScrollPlugin = jindo.$Class({
    /* @lends jindo.m.DynamicScrollPlugin.prototype */
    /**
        초기화 함수
    ...
    **/
    ...
    /**
        jindo.m.DynamicScrollPlugin 에서 사용하는 모든 인스턴스 변수를 초기화한다.
    **/
    ...
    **/
    ...
    <V1.1.0 이후 수정사항>
    http://devcode.nhncorp.com/news/detail.php?news_id=329

    <1.2.0 이후 수정사항>
    http://devcode.nhncorp.com/news/detail.php?news_id=331

    <V1.3.0 이후 수정사항>
    . duration이 0일 경우, 바로 위치변경하도록 변경

    <V1.3.1 이후 수정사항>
    . TransitionProperty이 translate(true)일 경우에만 설정되도록 변경
    . isAndroid => isPositionBug, isIos로 구분하여 처리
    . _bUseHighlight => bUseHighlight 로 변경
    . sListElement, nRatio 옵션 추가 (jindo.m.DynamicScrollPlugin 추가)
    . bUseTransition 옵션 방어코드 제거
    . 젤리빈(4.1) 대응코드 추가
    . DynamicScroll을 사용한다고 할경우, bUseCss3d는 항상 false

    <V1.3.5 이후 수정사항>
    . bUseBounce : false일 경우, 스크롤을 더이상 할수 없을 경우, 시스템 스크롤이 발생하는 기능
    . [JINDOSUS-623] jindo.m.Flicking과 jindo.m.Scroll 사용시 bUseBounce가 true일 경우, bounce 원복되지 않는 문제
    . 가로 스크롤일경우, 터치 위치의 y가 30보다 작을경우 스크롤이 안되는 버그 수정
    <1.5.0 릴리즈>
    . touchStart, touchMove, touchEnd에서 이벤트 중지할 경우, 뒤이벤트 안타도록 수정
    . 윈도우 폰 대응
    . isClickBug 추가
*/
/**
    ...
    @extends jindo.m.UIComponent
    @uses jindo.m.Touch
    @uses jindo.m.DynamicScrollPlugin{,1}
    @keyword scroll, 스크롤
    @group Component
    ...
    **/
	_createDynamicScrollPlugin : function(sDirection) {
		if(!this._oDynamicScrollPlugin) {
			...
			this._oDynamicScrollPlugin = new jindo.m.DynamicScrollPlugin(this._htWElement["wrapper"], {
...
jindo.m.SlideFlicking= jindo.$Class({
    ...
    **/
    ...
    _initCoreScroll : function(){
        this._oCore = new jindo.m.CoreScroll(this._htWElement.base.$value(),{
             bUseHScroll : this.bH,
             bUseVScroll : !this.bH,
             bUseMomentum : false,
             bUseCss3d : this._htOption['bUseCss3d'],
             bUseTimingFunction : this._htOption['bUseTimingFunction'],
             bUseTranslate : this._htOption['bUseTranslate'],
             bUseHighlight : this._htOption['bUseHighlight']
        });
        
        //console.log(this._htOption['bUseCss3d'], this._htOption['bUseTimingFunction'],  this._htOption['bUseTranslate']);
    ...
    _updateSizeInfo : function(){
        ...
        for(var i=0,nLen = this._htWElement.aPanel.length; i<nLen;i++){
            if(i != 0){
                if(this._htOption['nFlickDistanceOffset'] === null){
                    nPos += this._htWElement.aPanel[i-1][sLen]()*-1;
                }else{
                    var nW = this._htWElement.aPanel[i-1][sLen]()*-1;
    ...
    _setElementSize : function(){
       ...
       if(this._htOption['bAutoSize']){   
           if(this.bH){
               //this._htWElement.container.width(0);
                nMaxSize = nW*nLen;
    ...
    /**
     *  @description CoreScroll의 'beforeTouchStart'
     */
    _onBeforeTouchStart : function(oCustomEvt){
        ...
        if(!this._WrapperInstacne._fireCustomEvent('touchStart',htParam)){
            oCustomEvt.stop();
            return;
        }
    },
    ...
    /**
     * @description CoreScroll의 'beforeTouchMove/beforeTouchEnd'
     */
    ...
    /**
     * @description 코어 스크롤에서 touchEnd 시점에 발생한다. 플리킹될 다음 포지션을 잡아준다.
     */
    _onBeforeScrollEnd : function(oCustomEvt){
        ...
        var htParam = {
            nContentsIndex : this._WrapperInstacne.getContentIndex(),
            nContentsNextIndex : htPos.nIndex
        };
        
        if(this.bH){
            htParam.bLeft = htPos.nFlickLeft;
        }else{
            htParam.bTop = htPos.nFlickLeft;
        }
        
        
        if(!this._WrapperInstacne._fireCustomEvent('beforeFlicking',htParam)){
            oCustomEvt.stop();
            return;
        }
    ...
    /**
     * @description 코어스크롤이 스크롤이 끝난 시점 처리한다. 
     */
    _onScrollEnd : function(){
        //console.log('scrollEnd');
        var nBeforeContent = this._WrapperInstacne._htIndexInfo.nContentIndex;
        //console.log('_onScrollEnd ', this._bMoveEvent);
        if( (this._WrapperInstacne._htIndexInfo.nContentIndex !==  this._WrapperInstacne._htIndexInfo.nNextContentIndex) && (this._bFlickEvent || this._bMoveEvent) ){
            this._WrapperInstacne._htIndexInfo.nContentIndex =  this._WrapperInstacne._htIndexInfo.nNextContentIndex;
            ...
            if(this.bH){
                htParam.bLeft = this._WrapperInstacne._bFlickLeft;
            }else{
                htParam.bTop = this._WrapperInstacne._bFlickLeft;
            }
    ...
    */
      _getSnap : function(nX, nY, nDisX, nDisY, nMomX, nMomY){
        ...
        if((Math.abs(nDis) <=  this._htOption['nFlickThreshold']) ){
            // if(!(!jindo.m.getDeviceInfo().android && nMom != 0)){
                nIndex = this._WrapperInstacne.getContentIndex();
            // }
        }
        ...
        // var nTime = ( !jindo.m.getDeviceInfo().android && (nMom != 0))? Math.round(this._htOption['nDuration']*nGap*0.5) : this._htOption['nDuration'];       
        // nTime = Math.min(nTime, this._htOption['nDuration']);
       ...
       if(nDis < 0 ){ //왼쪽 또는 아래쪽 
        ...
        return {
            nX : this.bH? nNewPos : nPosX,
            nY : this.bH? nPosY : nNewPos,
            nTime : Math.round(nTime),
            nIndex : nIndex,
            nFlickLeft : bFlickLeft
        }
    ...
    _getPosition :function(nIndex){
        ...
        return {
            nX : this.bH? this._htPosition[nIndex]: nPosX,
            nY : this.bH? nPosY : this._htPosition[nIndex]
        }       
    },
 ...
 * @param {Boolean} bFireFlickEvent afterFlicking 커스텀 이벤트 발생여부
     */
    _moveTo : function(n, nDruation, bFireEvent, bFireFlickEvent){
        var htPos = this._getPosition(n);
        
        this._WrapperInstacne._htIndexInfo.nNextContentIndex = n;
        var nCurrent = this._WrapperInstacne.getContentIndex();
        if(nCurrent < n){
            this._WrapperInstacne._bFlickLeft = true;
        }else{
            this._WrapperInstacne._bFlickLeft = false;
        }
        
        //this._WrapperInstacne._bFlickLeft
 ...
 * @param {Object} nDuration 애니메이션 시간
     */
    moveNext : function(nDuration){
        var n = this._WrapperInstacne.getNextIndex();
       
        
        if(!this._WrapperInstacne._fireCustomEvent('beforeFlicking',{
            nContentsIndex : this._WrapperInstacne.getContentIndex(),
            nContentsNextIndex  : n
        })){
            return;
 ...
 * @param {Object} nDuration 애니메이션 시간 
    */
    movePrev : function(nDuration){
        var n = this._WrapperInstacne.getPrevIndex();
	...
	**/
	_drawCalendarBodyHtml : function() {
		var aHTML = [],oDate, nFirstTime, nLastTime,
			bPaintLastDay = false,
			nNowTime, aClassName, nNowDate, nDay;
			// bPrevMonth, bNextMonth;
...
jindo.m.CheckBox = jindo.$Class({
	...
	**/
	...
	<1.1.1 릴리즈 코드 수정>
	- nBounceDuration옵션 추가
	- 크롬 대응 코드 추가
	- android 4.0에서 세로 플리킹 대응 코드 추가
	- bUseCss3d 옵션 추가
	<1.1.4 릴리즈 코드 수정>
	- LG 단말기 대응
	- beforeFlicking 에서 stop() 호출시에 바운스되어 되어 되돌아감
	<1.5.0 릴리즈>
	- requestAnimationFrame/cancelAnimationFrame 추가
**/
/**
	...
	@extends jindo.m.UIComponent
	@uses jindo.m.Touch
	@uses jindo.m.CircularSlideFlicking
	@deprecated
...
jindo.m.CoreTab = jindo.$Class({
    ...
    **/
	...
	<V1.1.0 이후>
	 1. chrome for android에 대응

	<1.2.0 이후>
	 . datepicker에서 jindo.m.Calendar 제거.
	   (기존 jindo.Calendar => jindo.m.Calendar로 변경)
	 . draw 관련 사용자 이벤트 제거
	   (beforeDraw, Draw, afterDraw)
**/
/**
...
/**
    ...
    <1.1.0 이후 변경 사항>
    1. Android 4.0 랜더링 버그
      모바일 기기 가로에서 세로로 바뀔경우, 화면의 위치는 정상이나, 화면이 랜더링 되지 않는 버그
      강제로 reflow를 발생.
    <1.2.0 이후 변경사항>
      ios position:fixed 사용시 화면 구성후 최소 500ms 이상을 지연해야 html또는 body의 사이즈가 정상적으로 나오는 문제 (ios의 position:fixed 제거)
      position:fixed 적용 범위 수정 (jindo.m으로 이관)
**/
/**
...
/**
    ...
    <1.1.0 이후 수정사항>
    1. fixed 속성이 지원될 경우, floatingLayer를 hide시키지 않는다.
    <1.2.0 이후 수정사항>
    .bUseHideUI 옵션추가
    .플로팅 레이어 사라진 이후, 사라진 영역에서 이벤트가 발생하지 않는 현상 수정
    .bUseFixed 옵션 추가
    .갤럭시s2,s 안정화 작업 (다중 fade-in 발생시 발생안되도록 수정 (깜빡임))
    <1.3.0 이후 수정사항>
    .갤럭시s3인 경우, hide 이후 랜더링 버그가 있음. (수정)
    .삼성 4.x 에서 touchstart 이벤트가 발생되지 않는 버그 (document에 attach를 document.body에 attach)

    <1.3.5 이후 수정사항>
    . FloatingLayer의 크기가 %로 지정되었을 경우, %로 지정하도록 수정
     (http://devcode.nhncorp.com/projects/jindo-mobile-ui/issue/3367)
**/
/**
...
jindo.m.FloatingLayer = jindo.$Class({
    ...
    **/
    _onScrollEnd : function(we) {
        if(jindo.m.getDeviceInfo().iphone || jindo.m.getDeviceInfo().ipad ) {
            if(jindo.m._getVertical() != jindo.m.isVertical()) {
                return;
            }
        }
    ...
    **/
    ...
    @fileOverview 페이지의 고정영역 내부를 터치하여 스크롤링 할 수 있고, pull down/up upload 기능 및 스크롤바가 있는 컴포넌트
    @author sculove
    @version 1.5.0
    @since 2011. 9. 7.

    <2차 Release이후, 코드 수정사항> http://dev.naver.com/news/detail.php?news_id=732
    <V1.0.0 이후 수정사항>    http://devcode.nhncorp.com/news/detail.php?news_id=325
    <v1.1.0 이후 수정사항>
        1. UI변경
            pullDown이나 pullUp 상태가 아닌 경우, pullDown,pullUp 엘리먼트 hide 시킴
        2. 메소드 추가
            가로, 세로 스크롤 여부 반환 hasHScroll, hasVScroll
        3. 옵션 추가
            nOffsetBottom, nOffsetBottom
        4. 옵션명 변경
            bUseTransition을 bUseCss3d 로 변경
    <1.2.0 이후 수정사항>
        . scrollbar border와 color 지정 옵션 추가
        . scrollbar refresh Core의 height,width가 0일 경우 예외처리
        . wrapper와 scroller가 동일하고 bUseBounce가 true인 경우 스크롤바를 생성하지 않는다.
    <V1.3.1 이후 수정사항>
        . bUseTranslate, bUseTimingFunction이 적용될수 있도록 변경
        . isAndroid => isPositionBug로 구분하여 처리
        . 스크롤바 border-radius, opacity 제거
        . 스크롤바 fade-in/out 효과 제거
    <2012.10.24 oyang 수정>
        - nScrollbarHideThreshold 옵션을 추가하여 스크롤 바를 hide 시킬때 딜레이를 시킴
        - bUseScrollBarRadius 옵션을 추가하여 radius 설정을 옵션으로 처리.
**/
/**
    페이지의 고정영역 내부를 터치하여 스크롤링 할 수 있고, pull down/up upload 기능 및 스크롤바가 있는 컴포넌트
    ...
    @history 1.5.0 Support Window Phone8 지원
    @history 1.5.0 Update [bUseScrollBarRadius] 옵션 추가
    @history 1.5.0 Update [nScrollbarHideThreshold] 옵션 추가
    @history 1.4.0 Support iOS 6 지원
    @history 1.4.0 Update bUseBounce : false일 경우, 스크롤을 더이상 할수 없을 때 시스템 스크롤이 발생하는 기능 추가
    @history 1.4.0 Bug 가로 스크롤일경우, 터치 위치의 y가 30보다 작을경우 스크롤이 안되는 버그 수정
    ...
    @history 1.3.5 Update sScrollbarBorder Option 기본값 수정 ("1px solid rgba(255,255,255,0.9)" → "1px solid white")<br />
                sScrollbarColor Option 기본값 수정 ("rgba(0,0,0,0.5)" → "#8e8e8e")
    ...
    @history 1.2.0 Support Chrome for Android 지원<br />갤럭시 S2 4.0.3 업데이트 지원
    @history 1.2.0 Update pullDown/pullUp 상태가 아닌 경우, pullDown/pullUp 엘리먼트를 hide시키는 UI 변경<br />
                nOffsetTop Option 추가<br />
                nOffsetBottom Option 추가
    ...
    @history 0.9.5 Bug iOS에서 클릭영역 누른 상태에서, 이동후 버튼을 놓았을시, 초기에 선택한 위치에 clickable 엘리먼트가 존재할 경우, click 되는 문제 해결
    @history 0.9.5 Update [bUseFixedScrollbar] Option 추가<br />
                    [bUseFocus] Option명 삭제<br />
                    [sPullDownId] Option명 삭제<br />
                    [sPullUpId] Option명 삭제
    @history 0.9.0 Release 최초 릴리즈
**/
...
/* @lends jindo.m.Scroll.prototype */
    /**
        ...
        @constructor
        @remark
            jindo.m.Scroll은 내부적으로 <auidoc:see content="jindo.m.CoreScroll">jindo.m.CoreScroll</auidoc:see>을 사용하고 있다.<br />
            <auidoc:see content="jindo.m.CoreScroll">jindo.m.CoreScroll</auidoc:see>에서 사용하는 옵션은 <auidoc:see content="jindo.m.CoreScroll">jindo.m.CoreScroll</auidoc:see>의 옵션을 참조하기 바란다.
        @param {String|HTMLElement} el Scroll할 Element (필수)
        @param {Object} [htOption] 초기화 옵션 객체
            @param {Boolean} [htOption.bActivateOnload=true] 컴포넌트 로드시 activate 여부
            @param {Boolean} [htOption.bUseScrollbar=true] 스크롤바 사용 여부
            @param {Boolean} [htOption.bUseFixedScrollbar=false] 고정 스크롤바 적용 여부
            @param {String} [htOption.sScrollbarBorder="1px solid white"]
            @param {String} [htOption.sScrollbarColor="#8e8e8e"]
            ...
            @param {String} [htOption.sClassPrefix="scroll_"]
            @param {Boolean} [htOption.bUseVScroll=true]
            @param {Boolean} [htOption.bUseHScroll=false]
            @param {Boolean} [htOption.bUseBounce=true]
            ...
            @param {Number} [htOption.fnPullUpUpdating=null] bUsePullUp이 true일 시, pullUp 발생 시 엘리먼트를 구성하는 함수.<br />
                첫번째 파라미터로 pullUp의 jindo.$Element가 넘어져 온다.
            @param {Number} [htOption.nOffsetTop=0] 스크롤 시작 포지션에 offset을 지정한다.<br />
                Top을 기준으로 양수일 경우, 스크롤의 시작 포지션이 아래로 내려가고, 음수일 경우, Top을 기준으로 스크롤의 끝 포지션이 위로 올라간다.
            @param {Number} [htOption.nOffsetBottom=0] 스크롤 끝 포지션에 offset을 지정한다.<br />
                Bottom을 기준으로 양수일 경우, 스크롤의 끝 포지션이 위로 올라가고, 음수일 경우, Bottom을 기준으로 스크롤의 끝 포지션이 아래로 내려간다.
    ...
    **/
    ...
    /**
        pull down/up 관련 함수 옵션이 설정되지 않았을 경우 초기화한다.
    **/
    ...
    **/
    ...
    /**
        move시 duration을 지정한 후 이벤트. setDuration
        @param {jindo.$Event} we
    **/
    _onSetDuration : function(we) {
        if (this.option("bUseScrollbar")) {
            if (we.bUseHScroll && this._htWElement["HscrollbarIndicator"]) {
                this._htWElement["HscrollbarIndicator"].css("-" + this._oCore.sCssPrefix + "-transition-duration", we.nDuration);
            }
            if (we.bUseVScroll && this._htWElement["VscrollbarIndicator"]) {
                this._htWElement["VscrollbarIndicator"].css("-" + this._oCore.sCssPrefix + "-transition-duration", we.nDuration);
            }
        }
    },

    /**
        beforeTouchStart 이벤트
        @param {jindo.$Event} we
    **/
    _onBeforeTouchStart : function(we) {
        /**
            ...
            @param {jindo.$Event} oEvent touchStart 이벤트 객체
            @param {Function} stop 수행시 touchStart 이벤트가 발생하지 않음
            @history 1.5.0 Bug touchStart 시 이벤트를 막는 경우 touchMove, touchEnd가 발생하는 문제 수정
            @history 1.3.0 Bug stop시 이벤트가 중지되지 않는 문제
        **/
...
//      console.log("이동..." + we.nLeft + ", " + we.nTop + " max : " + we.nMaxScrollTop + " ,,, " + this._htWElement["pullDown"]._isReady_);
    ...
    **/
    _onBeforePosition : function(we) {
        /**
            ...
            @history 1.3.0 Bug stop시 이벤트가 중지되지 않는 문제
        **/
        if(!this.fireEvent("beforePosition",we)) {
            we.stop();
        }
    },

    /**
        스크롤 포지션 설정 후 이벤트 position
        @param {jindo.$Event} we
    **/
    _onPosition : function(we) {
        if(this.option("bUseScrollbar")) {
            if(this._nScrollBarHideTimer){
                clearTimeout(this._nScrollBarHideTimer);
    ...
    **/
    _hideScrollBar : function(sDirection) {
        ...
        if(bUseScroll && wel) {
            if(this._oCore.isPositionBug && this._oCore.bUseHighlight) {
                this._oCore.makeStylePos(this._htWElement[sDirection + "scrollbarIndicator"]);
            }
            wel.hide();
            /* 갤럭시 S3인 경우 hide된 후 reflow가 발생하지 않으면 스크롤바가 사라지지 않는다. */
    ...
    **/
    _fireEventPullDown : function() {
        ...
        /**
            ...
            @event pullDown
            @param {String} sType 커스텀 이벤트명
            @param {jindo.$Element} welElement (jindo.$Element) : pullDown 엘리먼트
            @param {jindo.m.Scroll} oScroll jindo.m.Scroll 엘리먼트
            @remark
                pull down 시 Scroller의 크기(DOM의 수 증감, DOM의 크기 증감)가 변경되었을 경우.
                변경 후, 꼭! oScroll.refresh()를 호출하여야 한다.
        **/
    ...
    **/
    _fireEventPullUp : function() {
        if(!this._htWElement) {
            ...
            @event pullUp
            @param {String} sType 커스텀 이벤트명
            @param {jindo.$Element} welElement (jindo.$Element) : pullUp 엘리먼트
            @param {jindo.m.Scroll} oScroll jindo.m.Scroll 엘리먼트

            @remark
                pull up 시 Scroller의 크기(DOM의 수 증감, DOM의 크기 증감)가 변경되었을 경우.
                변경 후, 꼭! oScroll.refresh()를 호출하여야 한다.
        **/
    ...
    **/
    _refreshScroll : function(sDirection) {
        ...
        if(sDirection === "H") {
            if(!this._oCore.bUseHScroll || this._oCore.nWrapperW == this._oCore.nScrollW) {
                return;
            }
        } else {
            if(!this._oCore.bUseVScroll || this._oCore.nWrapperH == this._oCore.nScrollH) {
                return;
            }
        }
    ...
    **/
    _createScrollbarIndicator : function(sDirection) {
        ...
        if(this._oCore.isIos && this.option('bUseScrollBarRadius')) {
            welScrollbarIndicator.css("-" + this._oCore.sCssPrefix + "-border-radius", "12px");
        }
        ...
        if(this._oCore.option("bUseTranslate") || this._oCore.option("bUseCss3d")) {
            welScrollbarIndicator.css("-" + this._oCore.sCssPrefix + "-transition-property", "-" + this._oCore.sCssPrefix + "-transform")
                .css("-" + this._oCore.sCssPrefix + "-transform", "translate" + this._oCore.sTransOpen + "0,0" + this._oCore.sTransEnd);
        }
        if(this._oCore.option("bUseTimingFunction")) {
            welScrollbarIndicator.css("-" + this._oCore.sCssPrefix + "-transition-timing-function", "cubic-bezier(0.33,0.66,0.66,1)");
        }
        if(sDirection === "H") {
    ...
    **/
    _setScrollBarPos: function (sDirection, nPos) {
        ...
        if (welIndicator) {
            if(this._oCore.option("bUseTranslate")) {
                if (this._oCore.isPositionBug && this._oCore.bUseHighlight)  {
...
jindo.m.IndexScroll = jindo.$Class({
    ...
    **/
    showIndex : function() {
        this._bUseIndex = true;
        this._setPosFixedIndex(this._oCore._nTop);
...
/**
	...
	@extends jindo.m.Component
	@uses jindo.m.Scroll
	@uses jindo.m.AlignFlipFlicking, jindo.m.CircularSlideFlicking, jindo.m.CoverFlicking, jindo.m.FlickingAnimation, jindo.m.FlipFlicking, jindo.m.SlideFlicking {0,}
    ...
    **/
    resize : function() {
        ...
        if(nPrePageWidth > this._nPageWidth) {  // 기존 사이즈 보다 작아지는 경우
            ...
            this._htWElement["tab_container"].width(this._nPageWidth * nthis._nTotalPage);
	...
	**/
	...
	1. Tab 구조 변경
	 - jindo.m.CoreTab 상속
	  (패널 슬라이드 기능 추가)
	2. 기능 추가
	 - '더보기'탭 기능 추가 (sMoreText, nCountOnList 옵션 추가)
	3. 클래스 taba 추가
**/
/**
...
jindo.m.TextInput = jindo.$Class({
	...
	**/
	...
	<1.1.1 릴리즈 추가>
	- 초기에 'change'이벤트 발생하지 않게 수정.
	- change 이벤트는 실제로 transitionEnd시점에 발생하게 수정.
	- 오타 수정
	- nDuration 옵션추가
**/
/**
